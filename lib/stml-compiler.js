...
/*
var s = `@for($i = 0; i <= 32; i--){
  span
}`
var forRegex = /@for\(((\$[\w][\w\d]*)[ ]*=[ ]*([\d]*))[ ]*;[ ]*(([\w][\w\d]*)[ ]*([<=>!]*)[ ]*([\d\w]*))[ ]*;[ ]*((\w*)[ ]*([\+\=\-\*\/]*)[ ]*([\d\w]*))\)\{([\n\r ]*.*[\n\r ]*)\}/
var res = s.match(forRegex);
console.log(res)
...
class StimuleCompiler {
  constructor(inFile, outFile, opts) {
    ...
    this.opts         = { minify: opts.minify || false };
    this.lines        = [];
    this.cTags        = [];
    this.tags         = [];
    this.closingTags  = [];
    ...
    this.styles       = [];
    this.forLoops     = [];
    this.errorHandler = new ErrorHandler.handler(this);
